...
## 2024-10-24 - Array Deduplication Performance Bottleneck in NLP Token Matching
**Learning:** Using `Array.includes()` inside loops for deduplication in NLP token matching (e.g., `calculateAffinity`) causes an O(N^2) performance bottleneck.
**Action:** Always use a `Set` for tracking common keys and `Set.has()` for constant-time lookups to maintain optimal execution speed when performing lookups inside loops.
