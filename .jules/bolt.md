...
## 2024-07-26 - NLP Deduplication Performance Bottleneck
**Learning:** Using `Array.includes()` for deduplication inside loops (e.g. `calculateAffinity` token matching) causes an O(N^2) performance bottleneck.
**Action:** Always use a `Set` for tracking common keys and `Set.has()` (or `Set.add()`) for constant-time lookups to maintain optimal execution speed.
