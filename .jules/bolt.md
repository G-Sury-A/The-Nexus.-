...
## 2024-11-20 - Array.includes() O(N^2) Bottleneck in NLP
**Learning:** Using `Array.includes()` inside loops for deduplication in NLP token matching (e.g., `calculateAffinity`) causes an O(N^2) performance bottleneck. Similarly, repeatedly allocating arrays like `commonStarters` inside a loop degrades performance.
**Action:** Always use a `Set` for tracking common keys and `Set.has()` for constant-time lookups to maintain optimal execution speed, and move static allocations outside of loops.
