...
**Learning:** Using `.filter().sort()` inside a loop (e.g., iterating through `topSubjects` and sorting `matchingArticles`) causes an O(M * N log N) performance bottleneck, allocating new arrays and performing repeated sorting for each iteration.
**Action:** Pre-sort the target array outside of iterative loops. Inside the loop, use `.find()` to achieve O(N) first-match lookups instead of allocating new arrays via `.filter().sort()`.
