...
## 2026-09-01 - Intermediate Array Allocation in tokenize
**Learning:** Avoid using intermediate `.filter()` calls on arrays inside high-frequency functions (e.g., `tokenize`). Using `.filter(w => w.length > 3 && !STOP_WORDS.has(w))` creates an unnecessary intermediate array allocation and Garbage Collection (GC) overhead when processing text blocks repeatedly.
**Action:** Replace `.filter()` inside string processing tasks with a single-pass `for` loop that evaluates the conditions directly and pushes into the result array to prevent unnecessary O(N) memory allocations and reduce Garbage Collection (GC) overhead.
