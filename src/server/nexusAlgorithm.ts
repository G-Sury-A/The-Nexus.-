...
function calculateAffinity(a: RawArticle, b: RawArticle): { score: number, commonKeys: string[] } {
  ...
  tokensA.forEach(t => {
    if (tokensB.has(t)) {
      // ⚡ Bolt Optimization: Using Set for commonKeys eliminates O(N^2) complexity from Array.includes(), drastically improving set intersection performance during high-volume array comparisons.
      commonKeysSet.add(t);
      score += 1;
    }
  });
