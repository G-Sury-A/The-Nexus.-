...
function tokenize(text: string): string[] {
  ...
  // ⚡ Bolt Optimization: Replace .filter() with a single-pass `for` loop
  // that pushes valid elements directly into the result array to minimize
  // Garbage Collection (GC) overhead in high-frequency string processing.
  const result: string[] = [];
...
function extractEntities(text: string): string[] {
  ...
  //
  // ⚡ Bolt Optimization: Using minRawLength avoids intermediate .filter() array allocations
  // before the loop, improving performance by evaluating length directly inside the iteration.
...
function calculateAffinity(a: RawArticle, b: RawArticle): { score: number, commonKeys: string[] } {
  ...
  // Optimization: use a Set for O(1) deduplication instead of Array.includes which is O(N) leading to O(N^2)
...
function scoreAgainstPersona(tokens: string[], entities: string[], prefTokens: Set<string>): number {
  ...
  // ⚡ Bolt Optimization: Use a traditional for loop instead of .forEach() to avoid function allocation
  // overhead on every element, significantly improving execution time in high-frequency scoring.
...
export async function generateNexusBriefing(userPrefs: any) {
  ...
  for (let i = 0; i < chainOrder.length; i++) {
    ...
    // ⚡ Bolt Optimization: Pre-sort the array by score once before the loop
    // to avoid O(M * N log N) performance bottlenecks from allocating new arrays
    // and sorting them in every iteration of the topSubjects loop.
    articleEntities.sort((a, b) => b.score - a.score);

    topSubjects.forEach(subject => {
      // Find highest scored article containing this subject
      // ⚡ Bolt Optimization: Use .find() for O(N) first-match lookup instead of
      // .filter().sort() which is O(N log N)
      const matchingArticle = articleEntities
        .find(ae => ae.entitiesSet.has(subject) && !usedArticleIds.has(ae.article.id));
    ...
    if (selectedChain.length > 0) {
      ...
      // and avoid unnecessary array allocations when only the first matching element is needed.
