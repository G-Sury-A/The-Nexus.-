...
export async function generateNexusBriefing(userPrefs: any) {
  ...
  for (let i = 0; i < chainOrder.length; i++) {
    ...
    // ⚡ Bolt Optimization: Pre-sort the array once to prevent O(M * N log N) overhead.
    // Instead of filtering and sorting repeatedly in the loop, we sort once
    // and use .find() to grab the first O(N) match.
